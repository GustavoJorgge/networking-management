...
const CardTitle = React.forwardRef<HTMLHeadingElement, HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) =>
    (<h3 ref={ref} className={`flex gap-2 text-1xl font-semibold leading-none tracking-tight justify-between ${className ?? ''}`} {...props} />)
);
...
const CardContent = React.forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (<div ref={ref} className={` p-1 ${className}`} {...props} />));
